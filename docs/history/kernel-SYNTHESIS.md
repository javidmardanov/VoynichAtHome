...
- **Primary measure per parameter point:** with a fixed, registered number of replicates N (seeds `0..N-1` of the parameter point's stream), the **median distance** over the N replicates, reported with the acceptance probability `P(d ≤ ε)` and its Wilson interval. The candidate acceptance rule after the first calibration (`docs/CALIBRATION.md`) is the tail-robust median rule: a point is compatible when its median replicate distance is at or below ε_med, where ε_med is a high quantile of the medians of N-subsets of the true generator's own self-distances, calibrated per family from several planted points. A single replicate inside a threshold is never a result: as millions of seeds accumulate, "at least one corpus fell within ε" becomes true by chance. The kernel returns per-replicate distances and fingerprints; the specimen seed is for pictures.
- **Search procedure:** coarse-to-fine. The fingerprint resolves parameter differences finer than any affordable coarse grid, so the registered search is a sequence of registered grid levels, each refined around the best region of the previous one, with a registered stopping rule. Every level keeps a complete ledger.
...
## 13. Go/no-go tests before the public platform (added after the second review)

The second review's assessment is accepted: software is no longer the main risk; scientific legitimacy, faithful implementations of the competing theories, and people who trust the project are. Its prior that this project contributes directly to an accepted decipherment is below one percent. That matches section 1: the platform's product is bounded compatibility knowledge, not a decipherment.

Three tests must pass before any investment in the public coordinator and contribute page (Gates 3 and 4):

1. **People.** A statistical-methods lead and a credible Voynich/domain advisor have joined, in addition to the custodian. Without a human statistical lead the registered experiment is not credible, whatever the code does.
2. **Compute.** The registered workload has been benchmarked and volunteer scale demonstrably adds something. The benchmark of the draft 2 kernel (`docs/BENCHMARK.md`) measures 0.04–0.08 s per simulation; the first experiment as outlined is an overnight job on a laptop, and a wide version is one day on one rented 32-core server. **The first experiment therefore runs on owned or rented hardware.** For experiment 1 the browser tier is a verification page (run one registered unit, check the hash), not capacity. A coordinator is built only for a registered workload that needs it.
3. **Commitments.** Twenty explicit pilot commitments obtained through direct outreach (Voynich Ninja, cipher communities, named researchers).

If the tests fail, the project stops at the local reproducible research engine plus the verification page. That is still worthwhile software and a still-publishable experiment; it is not Voynich@Home in the Folding@home sense, and it will not be called that.
...
**Draft 2.** Review incorporated as listed in section 0. Kernel changes: RFC 8785 canonical JSON with shared conformance vectors (`contracts/jcs-vectors.json`; the UTF-16 ordering vector caught an error in the author's own expectation, which is exactly what such vectors are for); strict validation of targets, layouts, seeds and parameters; NaN prohibited and negative zero canonicalised in outputs; results framed with `numeric_profile` and a replicate summary; `specimen_*` fields replace `best_*`; decimal-string parameters; structure-preserving parser with a per-locus round-trip test; deterministic quire partition and role-filtered targets; import/export allowlist and a randomised parity harness (100 random jobs, all identical on V8/x86-64). The committed target now comes from the discovery + validation quires (28,367 words, 3,306 lines). Sanity results against it (weighted z-distance, median over 3 replicates): manuscript discovery + validation 0.0; Currier A pages only 7.5; Currier B pages only 4.4; `bagofwords` 8.6; `charmarkov` 9.9; `selfcite` defaults 55; `gibberish` 63. These are development numbers on unregistered weights and are not results.

**After the second review.** Go/no-go tests recorded (section 13). Benchmark added (`docs/BENCHMARK.md`): 43–79 ms per simulation natively, 58–84 ms in WebAssembly, on the full 28,367-word layout; the first experiment does not need volunteer computing. Roadmap consequence: experiment 1 runs on one machine; the public coordinator is conditional on the three tests and on a workload that needs it.

**Gate 2 tooling (after the second review).** `voynich sweep`, `plant`, `calibrate` and `self-distance` added: parallel grid sweeps with complete ledgers, planted pseudo-manuscripts, and calibration of candidate acceptance rules with recovery, specificity and control checks. First results (`docs/CALIBRATION.md`): the tail-robust median rule recovers planted points and rejects all controls; the acceptance-probability rule fails under heavy-tailed generators; the replicate-cloud rule is too permissive; the search must be coarse-to-fine; the block-bootstrap scale is biased for repetition statistics and is a Gate 2 item for the statistician.

**Overnight batch, part 1.** Browser verification page (`web/verify`: manifest digest check before instantiation, one golden unit in a worker, hash comparison, specimen text; Playwright check in CI; Pages workflow awaiting the owner enabling Pages). Recruiting one-pager in English and Russian. Fifth generator family `slotgram` (slot grammar with a configurable table; default table flagged for expert verification; default parameters score 30 against the target, between the Markov control and the self-citation defaults).

**Overnight batch, part 2.** Candidate statistics for `fingerprint-v2` (`docs/FINGERPRINT-V2-CANDIDATES.md`): line-position effects, directional constraints, slot conformance. The word-length slope along the line separates the manuscript from the order-3 Markov control; slot conformance with the approximate table is 0.76 against Zattera's published 0.97, so the table must be corrected by the domain advisor.

**Overnight batch, part 3.** Subsampling scale estimator and a regularised Mahalanobis metric added as registered options (`build-targets --scale`, `--covariance-lambda`; per-unit `metric` field, default unchanged). Evaluation (`docs/CALIBRATION.md`, addendum): the scale estimator is not the source of the inflated self-distances; between-seed variance of path-dependent generators is, and rule C's per-family calibration handles it. Mahalanobis at λ = 0.5 improves control-to-self separation by about 20 %.

**Overnight batch, part 4.** Coarse-to-fine refinement (`voynich refine`): registered levels around the best point, declared integer axes, ledger per level, one random stream per parameter point across levels. It recovers the off-grid planted point exactly at level 1 as the unique compatible point (3,672 simulations). On the real target the self-citation family bottoms out near a median distance of 48, an order of magnitude from compatibility; a development result, not a finding. Six overnight items delivered: verification page, recruiting one-pager, slot-grammar family, fingerprint-v2 candidates, scale/Mahalanobis options, refinement.
