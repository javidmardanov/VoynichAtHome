...
2. Record the exact source commit, migrations, input sources and rights, module digest, campaign manifests, dependency audit, and reproduction results. Confirm the enabled sign-in method, the owner's account, deletion and revocation; verify Google/GitHub callbacks if those optional providers are enabled. Check provider usage, limits, and the reserve for already issued work.
