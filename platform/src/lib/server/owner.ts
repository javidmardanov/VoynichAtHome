...
import { backup, restore, portableBackup, importBackupObject } from './backup';
import { publishReport, requireRecoveryEvidence } from './reports';
export async function ownerAction(env:Env,actor:string,payload:unknown) {
  const action=z.discriminatedUnion('action',[
    z.object({action:z.literal('backup')}).strict(),
    z.object({action:z.literal('portable-backup')}).strict(),
    z.object({action:z.literal('import-backup-object'),object:z.unknown()}).strict(),
    z.object({action:z.literal('publish-report'),report:z.unknown()}).strict(),
    z.object({action:z.literal('withdraw-report'),digest:z.string().regex(/^sha256:[0-9a-f]{64}$/),reason:z.string().min(10).max(500)}).strict(),
  ...
  switch(action.action){
    ...
    case 'portable-backup':result=await portableBackup(env);break;
    case 'import-backup-object':result=await importBackupObject(env,action.object);break;
    case 'publish-report':result=await publishReport(env,action.report);break;
    case 'withdraw-report':await env.DB.batch([
      env.DB.prepare('UPDATE reports SET withdrawn=1,withdrawal_reason=? WHERE digest=?').bind(action.reason,action.digest),
      env.DB.prepare(`UPDATE campaigns SET scientific_status='computation' WHERE id IN (SELECT campaign_id FROM reports WHERE digest=?)`).bind(action.digest),
      env.DB.prepare(`UPDATE campaigns SET status='paused',updated_at=? WHERE status='active' AND json_extract(manifest,'$.kind')='manuscript'
        AND EXISTS (SELECT 1 FROM json_each(campaigns.manifest,'$.recovery_evidence') WHERE value=?)`).bind(now(),action.digest)
    ]);break;
    ...
    case 'revoke-release':await env.DB.batch([
      env.DB.prepare("UPDATE releases SET state='revoked' WHERE id=?").bind(action.id),
      env.DB.prepare("UPDATE reports SET withdrawn=1,withdrawal_reason='Underlying worker release revoked.' WHERE campaign_id IN (SELECT campaign_id FROM units WHERE release_id=?)").bind(action.id),
      env.DB.prepare("UPDATE campaigns SET scientific_status='computation' WHERE id IN (SELECT campaign_id FROM units WHERE release_id=?)").bind(action.id),
      env.DB.prepare(`UPDATE campaigns SET status='paused',updated_at=? WHERE status='active' AND json_extract(manifest,'$.kind')='manuscript'
        AND EXISTS (SELECT 1 FROM json_each(campaigns.manifest,'$.recovery_evidence') e JOIN reports r ON r.digest=e.value WHERE r.withdrawn=1)`).bind(now())
    ]);break;
    ...
    case 'campaign-state':{
      const campaign=await env.DB.prepare('SELECT status,manifest FROM campaigns WHERE id=?').bind(action.id).first<{status:string;manifest:string}>();
      ...
      if(action.status==='active'){
        ...
        if(manifest.kind==='manuscript')await requireRecoveryEvidence(env,manifest.recovery_evidence,manifest.methods,manifest.search_condition);
  ...
  const detail=action.action==='unit'?{campaign:action.campaign}:['campaign','import-backup-object','publish-report'].includes(action.action)?{imported:true,result}:action;
