...
self.onmessage=async({data})=>{
  try{
    ...
    const response=await fetch(approved.url);if(!response.ok)throw Error('Your browser could not load the software needed for this replay.');
    const bytes=new Uint8Array(await response.arrayBuffer());if(await sha256(bytes)!==approved.digest)throw Error('The downloaded replay software does not match its published digest.');
  ...
  }catch(e){self.postMessage({error:e instanceof Error?e.message:'The browser could not complete this replay.'});}
};
