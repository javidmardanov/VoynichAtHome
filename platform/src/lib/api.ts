export async function api<T=any>(path:string,payload?:unknown,signal?:AbortSignal):Promise<T> {
  ...
  try{
    ...
    const value=await response.json();if(!response.ok)throw Error(value.error??'The server could not complete the request. Try again.');return value;
  }catch(error){if(timedOut)throw Error('The server took too long to respond. Check your connection and try again.');throw error;}
